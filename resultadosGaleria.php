...
<body>
    ...
    <section class="galeria">
        ...
        <a class="resultadosVolver" href="galeria.php">Volver</a>

        <form action="resultadosGaleria.php?pagina=1" method="POST">

            <input class="inputBusqueda" type="text" required name="busquedaGaleria" placeholder="Buscar Obra">
            <button class="btn btn-dark">Buscar</button>
        </form>

        <?php
        if (!$_GET) {
            header('Location:resultadosGaleria.php?pagina=1');
        }

        include('model/conexion.php');

        $buscar = $_POST['busquedaGaleria'];
        echo "Búsqueda: <h6>" . $buscar . "</h6><br>";

        $obrasEnPaginas = 4;
        $iniciarPaginado = ($_GET['pagina'] - 1) * $obrasEnPaginas;

        $consulta = mysqli_query($conexion, "SELECT * FROM obrasgaleria WHERE titulo LIKE '%$buscar%' OR autor LIKE '%$buscar%' LIMIT $iniciarPaginado,  $obrasEnPaginas ");
        $consultaColumnas = mysqli_query($conexion, "SELECT * FROM obrasgaleria WHERE titulo LIKE '%$buscar%' OR autor LIKE '%$buscar%'");
        ...
        if ($totalObras == 0) {
            echo "Actualmente no hay resultados para su búsqueda";
        }
        ?>
        ...
        <div class="obrasContainer">
            ...
            ?>
                ...
                <div class="obra">
                    <div class="imagenObra"> <img src="<?php echo $resultados['imagen']; ?>" alt="<?php echo $resultados['titulo'] . " por " . $resultados['autor']; ?>"> </div>
